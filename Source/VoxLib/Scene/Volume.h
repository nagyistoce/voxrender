...
public:
     ...
     */
    Volume(boost::shared_array<UInt8> data = boost::shared_array<UInt8>( ),
    ...
    /** Spacing modifier */     
    void setSpacing(Vector<float,4> const& spacing) { m_spacing = spacing; VOX_CC }

    /** Extent modifier */     
    void setExtent(Vector<size_t,4> const& extent) 
    { 
        m_extent = extent; VOX_CC 
    }
    ...
    /** Data modifier */ 
    void setData(boost::shared_array<UInt8> const& data, 
...
private:
    ...
    boost::shared_array<UInt8> m_data;	  ///< Pointer to volume data

    Vector4f    m_spacing; ///< Spacing between voxels
